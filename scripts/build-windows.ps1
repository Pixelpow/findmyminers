...
# Garantit un dossier data VIDE : jamais de donnees locales (config, comptes, cles) dans le zip.
if (Test-Path "$rel\app\data") { Remove-Item -Recurse -Force "$rel\app\data" }
...
rem Mode auto-heberge : pas de login requis, sert tes propres mineurs.
set LOCAL_MODE=1
echo.
