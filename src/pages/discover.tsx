...
function recommendedProfile(miner: DiscoveredMiner, t: (fr: string, en: string) => string) {
  if ((miner.tempC || 0) >= 82) return t('Éco', 'Eco');
  if ((miner.powerW || 0) >= 120) return t('Équilibré', 'Balanced');
  return 'Turbo';
}
...
export default function DiscoverPage() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({ error: t('Échec de la découverte', 'Discovery failed') }));
        throw new Error(typeof data.error === 'string' ? data.error : t('Échec de la découverte', 'Discovery failed'));
      }
      ...
      setError(scanError instanceof TypeError
        ? t('Serveur injoignable — vérifie que l’application est bien lancée, puis réessaie.', 'Server unreachable — make sure the app is running, then try again.')
        : scanError instanceof Error ? scanError.message : t('Échec du scan', 'Scan failed'));
      setWizardMiners([]);
    } finally {
      setScanning(false);
    }
  ...
  const saveSelectedMiners = async () => {
    ...
    try {
      ...
      if (!configRes.ok) throw new Error('Lecture de la configuration impossible');
      ...
      if (!updateRes.ok) throw new Error('Enregistrement des mineurs impossible');

      setAddedIds(toCreate.map((miner) => miner.id));
      setExistingMiners([...currentMiners, ...toCreate]);
      setStep('done');
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : 'Enregistrement des mineurs impossible');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <Head><title>{t('Découverte', 'Discovery')} · FindMyMiners</title></Head>
      <div style={{ maxWidth: 1180 }}>
        <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginBottom: 22 }}>
          <StepChip number={1} label={t('Scanner', 'Scan')} active={step === 'scan'} done={step !== 'scan'} />
          <StepChip number={2} label={t('Sélection', 'Select')} active={step === 'select'} done={step === 'review' || step === 'done'} />
          <StepChip number={3} label={t('Vérification', 'Review')} active={step === 'review'} done={step === 'done'} />
          <StepChip number={4} label={t('Ajout', 'Add')} active={step === 'done'} done={step === 'done'} />
        </div>

        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 22 }}>
          <StatCard label={t('Mineurs configurés', 'Configured miners')} value={String(existingMiners.length)} sub={t('Déjà enregistrés dans cette organisation', 'Already saved in this organization')} accent="#6aa7ff" />
          <StatCard label={t('Découverts', 'Discovered')} value={String(wizardMiners.length)} sub={t('Dernier scan réseau', 'Last network scan')} accent="#f59e0b" />
          <StatCard label={t('Prêts à ajouter', 'Ready to add')} value={String(selectedMiners.length)} sub={duplicateCount ? `${duplicateCount} ${t('doublon(s) ignoré(s)', 'duplicate(s) skipped')}` : t('Aucun doublon détecté', 'No duplicate detected')} accent="#4ade80" />
        ...
        <div style={{ ...appCardStyle(28, '20px 22px'), marginBottom: 18 }}>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)', marginBottom: 6 }}>{t('Sous-réseau', 'Subnet')}</label>
              <input value={subnet} onChange={(event) => setSubnet(event.target.value)} placeholder={t('ex. 192.168.1', 'e.g. 192.168.1')}
                style={{ ...inputStyle, width: 170 }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)', marginBottom: 6 }}>{t('De', 'From')}</label>
              <input value={fromStr} onChange={(event) => setFromStr(event.target.value)}
                style={{ ...inputStyle, width: 70, textAlign: 'center' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)', marginBottom: 6 }}>{t('À', 'To')}</label>
              <input value={toStr} onChange={(event) => setToStr(event.target.value)}
                style={{ ...inputStyle, width: 70, textAlign: 'center' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)', marginBottom: 6 }}>{t('Port', 'Port')}</label>
            ...
            <button onClick={handleScan} disabled={scanning}
              ...
              {scanning ? t('Scan en cours…', 'Scanning…') : t('Scanner le réseau', 'Scan the network')}
            </button>
          </div>
        </div>
        ...
        {scanning && (
          <div style={{ ...appCardStyle(28, '40px 24px 32px'), textAlign: 'center', marginBottom: 18 }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--foreground)', marginTop: 18, fontWeight: 700 }}>{t('Scan du réseau…', 'Scanning the network…')}</p>
            <p style={{ fontSize: 12.5, color: 'var(--muted-2)', marginTop: 4 }}>{t('Sonde', 'Probing')} {subnet || 'local'}.{fromStr}–{toStr} {t('sur le port', 'on port')} {portStr}</p>
          </div>
        )}
        ...
        {!scanning && step === 'scan' && wizardMiners.length === 0 && (
          <div style={{ ...appCardStyle(28, '46px 24px'), textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 16, color: 'var(--foreground)', marginBottom: 6, fontWeight: 700 }}>{t('Scanner le réseau', 'Scan the network')}</div>
            <div style={{ fontSize: 13, color: 'var(--muted)' }}>{t('Détecte les ASIC (Antminer, Whatsminer, Avalon) et les Bitaxe, ignore les doublons, prépare l’import.', 'Detects ASICs (Antminer, Whatsminer, Avalon) and Bitaxe, skips duplicates, prepares the import.')}</div>
          </div>
        )}
        ...
        {!scanning && step === 'select' && (
          <div style={appCardStyle(28, '20px 22px')}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, gap: 12, flexWrap: 'wrap' }}>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)', margin: 0 }}>{t('Sélectionne les appareils à intégrer', 'Select the devices to add')}</h2>
                <p style={{ fontSize: 12.5, color: 'var(--muted)', margin: '6px 0 0' }}>{t('Les doublons sont détectés automatiquement et exclus par défaut.', 'Duplicates are detected automatically and excluded by default.')}</p>
              </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 5 }}>
                        ...
                        {miner.duplicate && <span style={{ fontSize: 10.5, color: '#f87171', padding: '2px 7px', borderRadius: 9999, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)' }}>{t('Déjà configuré', 'Already added')}</span>}
                      </div>
                    ...
                    <div style={{ fontSize: 11.5, color: 'var(--muted-2)', whiteSpace: 'nowrap' }}>{t('Profil conseillé', 'Suggested profile')} : {recommendedProfile(miner, t)}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 18 }}>
              <button onClick={() => setStep('scan')}
                style={ghostButton}>
                <ArrowLeft style={{ width: 14, height: 14 }} /> {t('Retour', 'Back')}
              </button>
              <button onClick={() => canReview && setStep('review')} disabled={!canReview}
                ...
                {t('Vérifier la sélection', 'Review selection')} <ArrowRight style={{ width: 14, height: 14 }} />
              </button>
            </div>
          </div>
        )}
        ...
        {!scanning && step === 'review' && (
          <div style={appCardStyle(28, '20px 22px')}>
            <div style={{ marginBottom: 16 }}>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)', margin: 0 }}>{t('Vérifie avant l’import', 'Review before import')}</h2>
              <p style={{ fontSize: 12.5, color: 'var(--muted)', margin: '6px 0 0' }}>{t('Ajuste les noms si besoin. Chaque mineur sélectionné sera ajouté activé et prêt pour le polling.', 'Adjust names if needed. Each selected miner is added enabled and ready for polling.')}</p>
            </div>
              ...
              {selectedMiners.map((miner) => (
                <div key={miner.id} style={{ border: '1px solid var(--border-1)', borderRadius: 18, background: 'rgba(255,255,255,0.02)', padding: '14px 16px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 1.2fr) minmax(220px, 1.8fr) minmax(140px, 1fr)', gap: 12, alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: 11.5, color: 'var(--muted)', marginBottom: 4 }}>{t('Appareil détecté', 'Detected device')}</div>
                    ...
                    <div>
                      <div style={{ fontSize: 11.5, color: 'var(--muted)', marginBottom: 4 }}>{t('Nom dans le dashboard', 'Name in the dashboard')}</div>
                      <input value={miner.customName} onChange={(event) => updateWizardMiner(miner.id, { customName: event.target.value })}
                        style={inputStyle} />
                    </div>
                    <div>
                      <div style={{ fontSize: 11.5, color: 'var(--muted)', marginBottom: 4 }}>{t('Profil suggéré', 'Suggested profile')}</div>
                      <div style={{ fontSize: 13, color: 'var(--foreground)' }}>{recommendedProfile(miner, t)}</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 18 }}>
              ...
              <button onClick={saveSelectedMiners} disabled={saving || !selectedMiners.length}
                ...
                {saving ? t('Ajout…', 'Adding…') : `${t('Ajouter', 'Add')} ${selectedMiners.length} ${t('mineur(s)', 'miner(s)')}`}
              </button>
            </div>
          </div>
        )}
        ...
        {!scanning && step === 'done' && (
          <div style={{ ...appCardStyle(28, '34px 24px'), textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 19, fontWeight: 700, color: 'var(--foreground)', marginBottom: 6 }}>{t('Intégration terminée', 'Onboarding complete')}</div>
            <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 18 }}>{addedIds.length} {t('mineur(s) ajouté(s) à ta configuration.', 'miner(s) added to your configuration.')}</div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>
              <button onClick={() => setStep('scan')}
                style={ghostButton}>
                <RefreshCw style={{ width: 14, height: 14 }} /> {t('Relancer un scan', 'Scan again')}
              </button>
              <Link href="/miners" style={{ ...primaryButton, textDecoration: 'none' }}>
                {t('Voir les mineurs', 'View miners')} <ArrowRight style={{ width: 14, height: 14 }} />
              </Link>
            </div>
          </div>
        )}
