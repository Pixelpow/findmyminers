...
export default function DiscoverPage() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({ error: 'Échec de la découverte' }));
        throw new Error(typeof data.error === 'string' ? data.error : 'Échec de la découverte');
      }
    ...
    <>
      ...
      <div style={{ maxWidth: 1180 }}>
        <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginBottom: 22 }}>
          ...
          <StepChip number={2} label="Sélection" active={step === 'select'} done={step === 'review' || step === 'done'} />
          <StepChip number={3} label="Vérification" active={step === 'review'} done={step === 'done'} />
          <StepChip number={4} label="Ajout" active={step === 'done'} done={step === 'done'} />
        </div>
        ...
        <div style={{ ...appCardStyle(28, '20px 22px'), marginBottom: 18 }}>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)', marginBottom: 6 }}>Sous-réseau</label>
              <input value={subnet} onChange={(event) => setSubnet(event.target.value)} placeholder="ex. 192.168.1"
                style={{ ...inputStyle, width: 170 }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)', marginBottom: 6 }}>De</label>
              <input value={fromStr} onChange={(event) => setFromStr(event.target.value)}
                style={{ ...inputStyle, width: 70, textAlign: 'center' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)', marginBottom: 6 }}>À</label>
        ...
        {scanning && (
          <div style={{ ...appCardStyle(28, '40px 24px 32px'), textAlign: 'center', marginBottom: 18 }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--foreground)', marginTop: 18, fontWeight: 700 }}>Scan du réseau…</p>
            <p style={{ fontSize: 12.5, color: 'var(--muted-2)', marginTop: 4 }}>Sonde {subnet || 'local'}.{fromStr}–{toStr} sur le port {portStr}</p>
          </div>
        )}
        ...
        {!scanning && step === 'select' && (
          <div style={appCardStyle(28, '20px 22px')}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, gap: 12, flexWrap: 'wrap' }}>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)', margin: 0 }}>Sélectionne les appareils à intégrer</h2>
                <p style={{ fontSize: 12.5, color: 'var(--muted)', margin: '6px 0 0' }}>Les doublons sont détectés automatiquement et exclus par défaut.</p>
              </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 5 }}>
                        ...
                        {miner.duplicate && <span style={{ fontSize: 10.5, color: '#f87171', padding: '2px 7px', borderRadius: 9999, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)' }}>Déjà configuré</span>}
                      </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 18 }}>
              ...
              <button onClick={() => canReview && setStep('review')} disabled={!canReview}
                ...
                Vérifier la sélection <ArrowRight style={{ width: 14, height: 14 }} />
              </button>
            </div>
          </div>
        )}
        ...
        {!scanning && step === 'review' && (
          <div style={appCardStyle(28, '20px 22px')}>
            <div style={{ marginBottom: 16 }}>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)', margin: 0 }}>Vérifie avant l’import</h2>
              <p style={{ fontSize: 12.5, color: 'var(--muted)', margin: '6px 0 0' }}>Ajuste les noms si besoin. Chaque mineur sélectionné sera ajouté activé et prêt pour le polling.</p>
            </div>
              ...
              {selectedMiners.map((miner) => (
                <div key={miner.id} style={{ border: '1px solid var(--border-1)', borderRadius: 18, background: 'rgba(255,255,255,0.02)', padding: '14px 16px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 1.2fr) minmax(220px, 1.8fr) minmax(140px, 1fr)', gap: 12, alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: 11.5, color: 'var(--muted)', marginBottom: 4 }}>Appareil détecté</div>
                    ...
                    <div>
                      <div style={{ fontSize: 11.5, color: 'var(--muted)', marginBottom: 4 }}>Nom dans le dashboard</div>
                      <input value={miner.customName} onChange={(event) => updateWizardMiner(miner.id, { customName: event.target.value })}
                        style={inputStyle} />
                    </div>
                    <div>
                      <div style={{ fontSize: 11.5, color: 'var(--muted)', marginBottom: 4 }}>Profil suggéré</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 18 }}>
              ...
              <button onClick={saveSelectedMiners} disabled={saving || !selectedMiners.length}
                ...
                {saving ? 'Ajout…' : `Ajouter ${selectedMiners.length} mineur${selectedMiners.length > 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
        )}
        ...
        {!scanning && step === 'done' && (
          <div style={{ ...appCardStyle(28, '34px 24px'), textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 19, fontWeight: 700, color: 'var(--foreground)', marginBottom: 6 }}>Intégration terminée</div>
            <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 18 }}>{addedIds.length} mineur{addedIds.length > 1 ? 's ont' : ' a'} été ajouté{addedIds.length > 1 ? 's' : ''} à ta configuration.</div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>
              <button onClick={() => setStep('scan')}
                style={ghostButton}>
                <RefreshCw style={{ width: 14, height: 14 }} /> Relancer un scan
              </button>
              <Link href="/miners" style={{ ...primaryButton, textDecoration: 'none' }}>
                Voir les mineurs <ArrowRight style={{ width: 14, height: 14 }} />
              </Link>
            </div>
          </div>
        )}
