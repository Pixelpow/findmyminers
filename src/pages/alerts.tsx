...
const TYPE_META: Record<string, { label: string; chip: string; dot: string }> = {
  thermal: { label: 'THERMIQUE', chip: 'bg-rose-500/10 text-rose-400 border-rose-500/20', dot: 'bg-rose-500 dot-glow-rose' },
  ...
  'daily-report': { label: 'RAPPORT', chip: 'bg-blue-500/10 text-blue-400 border-blue-500/20', dot: 'bg-blue-500' },
...
export default function AlertsPage() {
  ...
  return (
    <>
      <Head><title>Alertes · FindMyMiners</title></Head>

      {/* Dense KPI Row */}
      <section className="grid grid-cols-3 gap-4">
        <div className="nova-glass p-4 flex flex-col justify-between transition-transform hover:-translate-y-[1px]">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-3">Événements critiques</div>
          ...
          <div className="text-[11px] font-mono text-slate-500 mt-2">Incidents thermiques & pool</div>
        </div>
        <div className="nova-glass p-4 flex flex-col justify-between transition-transform hover:-translate-y-[1px]">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-3">À traiter</div>
          <div className="font-mono text-3xl text-slate-100 font-bold tracking-tight">{unresolvedCount}</div>
          <div className="text-[11px] font-mono text-slate-500 mt-2">Événements à examiner</div>
        </div>
        <div className="nova-glass p-4 flex flex-col justify-between transition-transform hover:-translate-y-[1px]">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-3">Total événements</div>
          <div className="font-mono text-3xl text-slate-100 font-bold tracking-tight">{events.length}</div>
          <div className="text-[11px] font-mono text-slate-500 mt-2">1000 derniers enregistrements</div>
        </div>
      </section>
      ...
      <div className="nova-glass flex flex-col overflow-hidden relative min-h-[420px]">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Rechercher mineur ou message..."
          ...
          {loading ? (
            ...
            <div className="py-24 text-center">
              ...
              <h3 className="text-slate-300 font-sans font-semibold mb-1 text-sm">
                {search || typeFilter ? 'Aucune alerte correspondante' : 'Tout va bien'}
              </h3>
              <p className="text-slate-500 font-sans text-xs mb-4 max-w-sm mx-auto">
                {search || typeFilter
                  ? 'Essaie un autre filtre ou une autre recherche.'
                  : 'Ta flotte fonctionne normalement. Les alertes apparaîtront ici en cas d’anomalie de température, de hashrate ou de connectivité.'}
              </p>
              {!search && !typeFilter && (
                <Link
                  ...
                  <div key={`${event.ts}-${index}`} className="flex gap-3 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className={`text-[9px] font-mono font-semibold border px-1 py-0.5 rounded ${meta.chip}`}>{meta.label}</span>
                        {event.resolved && (
                          <span className="text-[9px] font-mono font-semibold border px-1 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border-emerald-500/20">RÉSOLU</span>
                        )}
                        <span className="text-[10px] text-slate-500 font-mono ml-auto shrink-0">{formatTime(event.ts)}</span>
                      </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between gap-3 px-5 py-3 border-t border-white/5 bg-white/[0.01] font-mono text-[11px] text-slate-500">
            <span>
              {(currentPage - 1) * ROWS_PER_PAGE + 1}–{(currentPage - 1) * ROWS_PER_PAGE + paginated.length} sur {filtered.length}
            </span>
            <div className="flex items-center gap-3">
