...
/** Loosely-typed miner identity coming from config or the status endpoint. */
type ConfigMiner = { id: string; name: string; port?: number; ip?: string; model?: string };

/** Hardware block from /api/miner/status — mixed string/number fields. */
type Hardware = {
  MAC?: string;
  MPO?: string;
  WORKMODE?: string;
...
type StatusData = {
  miner?: { name?: string; model?: string; ip?: string };
  model?: string;
  firmware?: string;
  description?: string;
  offline?: boolean;
  ...
  devs?: Array<Record<string, unknown>>;
  pools?: Array<Record<string, string | number>>;
  hardware?: Hardware;
  rawStats?: unknown;
...
export default function MinerDetailPage() {
  ...
  const [miners, setMiners] = useState<ConfigMiner[]>([]);
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      ...
      const m = (json.miners || []).find((mn: ConfigMiner) => mn.id === id);
  ...
  const FAN_LABELS: Record<'low' | 'medium' | 'high', string> = { low: t('Basse', 'Low'), medium: t('Moyenne', 'Medium'), high: t('Haute', 'High') };
  ...
  const setFanPreset = async (preset: 'low' | 'medium' | 'high') => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || t('Erreur', 'Error'));
      }
      setFanMode(preset);
      setAutoMode(false);
      setFanFeedback(t(`Ventilation → ${FAN_LABELS[preset]} (${FAN_PRESETS[preset]}%)`, `Fan → ${FAN_LABELS[preset]} (${FAN_PRESETS[preset]}%)`));
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setFanFeedback(t(`Erreur : ${msg}`, `Error: ${msg}`));
    } finally {
      setFanLoading(false);
  ...
  const PERF_PRESETS = {
    low: { value: '0', watt: 65, label: t('Éco', 'Eco'), desc: t('Silencieux · 65 W', 'Quiet · 65 W') },
    normal: { value: '1', watt: 90, label: t('Normal', 'Normal'), desc: t('Standard · 90 W', 'Standard · 90 W') },
    high: { value: '2', watt: 140, label: t('Perf', 'Perf'), desc: t('Performance · 140 W', 'Performance · 140 W') },
  } as const;
  ...
  const setPerformanceProfile = async (preset: 'low' | 'normal' | 'high') => {
    ...
    try {
      ...
      setPerfMode(preset);
      setPerfFeedback(t(`Mode → ${PERF_PRESETS[preset].label} (${PERF_PRESETS[preset].watt}W)`, `Mode → ${PERF_PRESETS[preset].label} (${PERF_PRESETS[preset].watt}W)`));
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setPerfFeedback(t(`Erreur : ${msg}`, `Error: ${msg}`));
    } finally {
  ...
  const toggleAutoMode = async () => {
    ...
    try {
      ...
      if (!res1.ok) {
        ...
        throw new Error(err.error || t('Erreur smart-speed', 'smart-speed error'));
      }
      ...
      if (enabling) {
        ...
        if (!res2.ok) {
          ...
          throw new Error(err.error || t('Erreur target-temp', 'target-temp error'));
        }
      }
      setAutoMode(enabling);
      if (enabling) setFanMode(null);
      setAutoFeedback(enabling ? t('Auto activé · Cible 75°C · Ventilo auto', 'Auto on · Target 75°C · Fan auto') : t('Auto désactivé · Ventilo manuel', 'Auto off · Manual fan'));
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setAutoFeedback(t(`Erreur : ${msg}`, `Error: ${msg}`));
    } finally {
  ...
  const saveSettings = async () => {
    try {
      ...
      if (!res.ok) return;
      const json = await res.json();
      const updated = (json.miners || []).map((m: ConfigMiner) =>
  ...
  if (loading && !data) {
    return <div style={{ padding: 40, color: 'var(--muted)', fontSize: 14 }}>{t('Chargement…', 'Loading…')}</div>;
  }
  ...
  const eventLabel = (event: MinerEvent) => {
    if (event.category === 'alert') return t('Alerte', 'Alert');
    if (event.category === 'maintenance') return t('Maintenance', 'Maintenance');
    if (event.category === 'action') return t('Action', 'Action');
    return t('Système', 'System');
  };

  const formatEventTime = (ts: number) => {
    const diffMs = Date.now() - ts;
    if (diffMs < 60_000) return t('à l’instant', 'just now');
    if (diffMs < 3_600_000) return t(`il y a ${Math.floor(diffMs / 60_000)} min`, `${Math.floor(diffMs / 60_000)} min ago`);
    if (diffMs < 86_400_000) return t(`il y a ${Math.floor(diffMs / 3_600_000)} h`, `${Math.floor(diffMs / 3_600_000)} h ago`);
    return new Date(ts).toLocaleString();
  };
      ...
      <div style={{ maxWidth: 980, minHeight: '100%' }}>
        ...
        <div style={{ ...cardStyle, borderRadius: 28 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--muted)', marginTop: 2 }}>
                  {data?.model || miner?.model || t('Modèle inconnu', 'Unknown model')} · SHA256
                </div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={restart} disabled={actionLoading}
                ...
                {actionLoading ? '…' : t('Redémarrer', 'Reboot')}
              </button>
              <div style={{ position: 'relative' }}>
                <button
                  onClick={() => setMenuOpen((o) => !o)}
                  aria-label={t('Plus d’actions', 'More actions')}
                  style={{ width: 40, height: 40, padding: 0, background: menuOpen ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.04)', border: '1px solid var(--border-1)', borderRadius: 14, color: 'var(--foreground)', cursor: 'pointer' }}>
                  <MoreHorizontal style={{ width: 16, height: 16 }} />
                </button>
                {menuOpen && (
                  <>
                    <div onClick={() => setMenuOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 40 }} />
                    <div style={{ position: 'absolute', right: 0, top: 46, zIndex: 50, minWidth: 210, background: '#0f0f12', border: '1px solid var(--border-1)', borderRadius: 14, padding: 6, boxShadow: '0 12px 34px rgba(0,0,0,0.55)' }}>
                      {miner?.ip && (
                        <a
                          href={`http://${miner.ip}`}
                          target="_blank"
                          ...
                          style={{ display: 'block', padding: '9px 12px', borderRadius: 9, fontSize: 13, color: 'var(--foreground)', textDecoration: 'none', cursor: 'pointer' }}
                        >
                          {t('Ouvrir l’interface web', 'Open web interface')}
                        </a>
                      )}
                      <button
                        onClick={removeMiner}
                        disabled={removing}
                        style={{ display: 'block', width: '100%', textAlign: 'left', padding: '9px 12px', borderRadius: 9, fontSize: 13, color: '#f87171', background: 'transparent', border: 'none', cursor: removing ? 'not-allowed' : 'pointer' }}
                      >
                        {removing ? t('Suppression…', 'Removing…') : t('Retirer de la flotte', 'Remove from fleet')}
                      </button>
                    </div>
                  </>
                )}
          ...
          <div style={{ borderTop: '1px solid var(--border-1)', marginTop: 18, paddingTop: 18 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0 }}>
              <div style={{ borderRight: '1px solid var(--border-1)', paddingRight: 24 }}>
                <div style={labelStyle}>{t('Adresse MAC', 'MAC address')}</div>
                <div style={valueStyle}>{hardware?.MAC || '—'}</div>
              </div>
              <div style={{ borderRight: '1px solid var(--border-1)', padding: '0 24px' }}>
                <div style={labelStyle}>{t('Modèle', 'Model')}</div>
                <div style={valueStyle}>{data?.model || miner?.model || miner?.name || '—'}</div>
              </div>
              <div style={{ paddingLeft: 24 }}>
                <div style={labelStyle}>{t('Adresse IP', 'IP address')}</div>
        ...
        <div style={{ display: 'flex', marginBottom: 16, gap: 8, flexWrap: 'wrap' }}>
          {tabBtn('status', t('Statut', 'Status'))}
          {tabBtn('charts', t('Graphiques', 'Charts'))}
          {tabBtn('timeline', t('Historique', 'History'))}
          {tabBtn('settings', t('Réglages', 'Settings'))}
        </div>
          ...
          <div style={cardStyle}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0, paddingBottom: 18 }}>
              <div style={{ borderRight: '1px solid #27272a', paddingRight: 24 }}>
                <div style={labelStyle}>{t('Température', 'Temperature')}</div>
              ...
              <div style={{ borderRight: '1px solid #27272a', padding: '0 24px' }}>
                <div style={labelStyle}>{t('Consommation', 'Power')}</div>
                ...
                {hardware?.MPO && <div style={{ fontSize: 11.5, color: '#52525b', marginTop: 2 }}>{t('Limite', 'Limit')} : {hardware.MPO} W</div>}
              </div>
              <div style={{ paddingLeft: 24 }}>
                <div style={labelStyle}>{t('Ventilation', 'Fan')}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#fafafa' }}>
            ...
            {isOnline && (fanRpm > 0 || fanPct > 0) && (
              <div style={{ borderTop: '1px solid #27272a', marginTop: 18, paddingTop: 16 }}>
                <div style={{ ...labelStyle, marginBottom: 10 }}>{t('Ventilation (live)', 'Fan (live)')}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: '#52525b', marginTop: 2 }}>
                      {autoMode ? 'Auto' : fanMode ? t(`Manuel · ${FAN_LABELS[fanMode]}`, `Manual · ${FAN_LABELS[fanMode]}`) : t('Manuel', 'Manual')}
                    </div>
                  </div>
                </div>
              </div>
            ...
            {isOnline && data?.source !== 'axeos' && (
              <div style={{ borderTop: '1px solid #27272a', marginTop: 18, paddingTop: 16 }}>
                <div style={{ ...labelStyle, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                  {t('Contrôle ventilation', 'Fan control')}
                  {fanLoading && (
                ...
                {fanFeedback && (
                  <div style={{
                    ...
                    background: /^(Erreur|Error)/.test(fanFeedback) ? 'rgba(239,68,68,0.1)' : 'rgba(34,197,94,0.1)',
                    color: /^(Erreur|Error)/.test(fanFeedback) ? '#f87171' : '#4ade80',
                    border: `1px solid ${/^(Erreur|Error)/.test(fanFeedback) ? 'rgba(239,68,68,0.2)' : 'rgba(34,197,94,0.2)'}`,
                  }}>
                    {fanFeedback}
                  </div>
                )}
            ...
            {isOnline && data?.source !== 'axeos' && (
              <div style={{ borderTop: '1px solid #27272a', marginTop: 18, paddingTop: 16 }}>
                <div style={{ ...labelStyle, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                  {t('Profil de performance', 'Performance profile')}
                  {perfLoading && (
                    <div style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid #27272a', borderTopColor: '#f7931a', animation: 'spin 0.8s linear infinite' }} />
                  )}
                  {detectedPerfMode && !perfMode && (
                    <span style={{ fontSize: 10.5, color: '#52525b', fontWeight: 400 }}>
                      {t('Actuel', 'Current')} : {detectedPerfMode === 'low' ? t('Éco', 'Eco') : detectedPerfMode === 'high' ? 'Perf' : 'Normal'}
                    </span>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                {perfFeedback && (
                  <div style={{
                    marginTop: 8, fontSize: 12, padding: '6px 10px', borderRadius: 6,
                    background: /^(Erreur|Error)/.test(perfFeedback) ? 'rgba(239,68,68,0.1)' : 'rgba(34,197,94,0.1)',
                    color: /^(Erreur|Error)/.test(perfFeedback) ? '#f87171' : '#4ade80',
                    border: `1px solid ${/^(Erreur|Error)/.test(perfFeedback) ? 'rgba(239,68,68,0.2)' : 'rgba(34,197,94,0.2)'}`,
                  }}>
                    {perfFeedback}
                  </div>
                )}
            ...
            {isOnline && data?.source !== 'axeos' && (
              <div style={{ borderTop: '1px solid #27272a', marginTop: 18, paddingTop: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <button onClick={toggleAutoMode} disabled={autoLoading}
                    ...
                    <div style={{ textAlign: 'left' }}>
                      <div>{autoMode ? t('Auto activé', 'Auto on') : t('Activer Auto', 'Enable Auto')}</div>
                      <div style={{ fontSize: 10.5, fontWeight: 400, color: '#52525b', marginTop: 1 }}>
                        {t('Ventilation auto + cible 75°C', 'Auto fan + 75°C target')}
                      </div>
                    </div>
                    <div style={{
                ...
                {autoFeedback && (
                  <div style={{
                    marginTop: 8, fontSize: 12, padding: '6px 10px', borderRadius: 6,
                    background: /^(Erreur|Error)/.test(autoFeedback) ? 'rgba(239,68,68,0.1)' : 'rgba(168,85,247,0.1)',
                    color: /^(Erreur|Error)/.test(autoFeedback) ? '#f87171' : '#c084fc',
                    border: `1px solid ${/^(Erreur|Error)/.test(autoFeedback) ? 'rgba(239,68,68,0.2)' : 'rgba(168,85,247,0.2)'}`,
                  }}>
                    {autoFeedback}
                  </div>
                )}
            ...
            {chipTemps.length > 0 && (
              <>
                <div style={{ borderTop: '1px solid #27272a', paddingTop: 16 }}>
                  <div style={{ ...labelStyle, marginBottom: 10 }}>{t(`Températures des puces (${chipTemps.length} puces)`, `Chip temperatures (${chipTemps.length} chips)`)}</div>
            ...
            <div style={{ borderTop: '1px solid #27272a', marginTop: 18, paddingTop: 16 }}>
              <div style={{ ...labelStyle, marginBottom: 10 }}>{t('Diagnostic maintenance', 'Maintenance diagnostics')}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                {!data?.maintenanceInsights?.length && (
                  <div style={{ fontSize: 12.5, color: '#52525b' }}>{t('Aucun diagnostic de maintenance pour l’instant.', 'No maintenance diagnostics yet.')}</div>
                )}
              </div>
            </div>

            <div style={{ borderTop: '1px solid #27272a', marginTop: 18, paddingTop: 16 }}>
              <div style={{ ...labelStyle, marginBottom: 10 }}>{t('Records de difficulté', 'Difficulty records')}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12, marginBottom: 12 }}>
                <div style={{ background: '#0f0f12', border: '1px solid #27272a', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ fontSize: 11.5, color: '#71717a', marginBottom: 4 }}>{t('Meilleur diff du mineur', 'Miner best diff')}</div>
                  ...
                  <div style={{ fontSize: 11.5, color: '#52525b', marginTop: 4 }}>{data?.diffRecords?.minerRecord?.bestDiffAccountKey || t('Aucun compte', 'No account')}</div>
                </div>
                <div style={{ background: '#0f0f12', border: '1px solid #27272a', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ fontSize: 11.5, color: '#71717a', marginBottom: 4 }}>{t('Dernier diff du mineur', 'Miner last diff')}</div>
                ...
                <div style={{ background: '#0f0f12', border: '1px solid #27272a', borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ fontSize: 11.5, color: '#71717a', marginBottom: 4 }}>{t('Meilleur diff de la flotte', 'Fleet best diff')}</div>
                  ...
                  <div style={{ fontSize: 11.5, color: '#52525b', marginTop: 4 }}>{data?.diffRecords?.globalRecord?.bestDiffMinerName || t('Aucun record flotte', 'No fleet record')}</div>
                </div>
              </div>
              ...
              {data?.diffRecords?.accountRecords?.length ? (
                <div style={{ border: '1px solid #27272a', borderRadius: 10, overflow: 'hidden' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 0.8fr', gap: 12, padding: '10px 14px', background: '#111114', color: '#71717a', fontSize: 11.5, fontWeight: 600 }}>
                    <div>{t('Compte / Adresse', 'Account / Address')}</div>
                    <div>{t('Meilleur diff', 'Best diff')}</div>
                    <div>{t('Dernier diff', 'Last diff')}</div>
                    <div>Pool</div>
                  </div>
                ...
                <div style={{ fontSize: 12.5, color: '#52525b' }}>{t('Aucun record de diff enregistré pour ce mineur.', 'No diff record saved for this miner.')}</div>
              )}
            </div>
          </div>
        )}
        ...
        {tab === 'status' && (
          ...
          <div style={{ border: '1px solid #27272a', borderRadius: 10, overflow: 'hidden' }}>
            ...
            >
              <span>{t('Stats avancées', 'Advanced stats')}</span>
              ...
              <div style={{ padding: '14px 22px', background: '#18181b', borderTop: '1px solid #27272a' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 32px' }}>
                  {[
                    ...
                    [t('Statut du pool', 'Pool status'), activePool?.Status || '—'],
                    [t('Stratum actif', 'Stratum active'), activePool?.['Stratum Active'] ? t('Oui', 'Yes') : t('Non', 'No')],
                    [t('Diff du dernier share', 'Last share diff'), activePool?.['Last Share Difficulty']?.toLocaleString() || '—'],
                    [t('Meilleur share', 'Best share'), summary?.['Best Share']?.toLocaleString() || '—'],
                    [t('Erreurs matérielles', 'Hardware errors'), summary?.['Hardware Errors']?.toString() || '0'],
                    [t('Difficulté acceptée', 'Difficulty accepted'), summary?.['Difficulty Accepted']?.toFixed(0) || '—'],
                    [t('Difficulté rejetée', 'Difficulty rejected'), summary?.['Difficulty Rejected']?.toFixed(0) || '—'],
          ...
          <>
            ...
            {historyLoading && !history ? (
              ...
              <>
                ...
                <div style={cardStyle}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
                    {[
                      { label: t('Hashrate moyen', 'Avg hashrate'), value: `${history.stats.avgHashrate.toFixed(2)} TH/s` },
                      { label: t('Temp moyenne', 'Avg temp'), value: `${history.stats.avgTemp.toFixed(1)}°C` },
                      { label: t('Conso moyenne', 'Avg power'), value: `${history.stats.avgPower.toFixed(0)} W` },
                      { label: t('Rejetés', 'Rejected'), value: `${history.stats.rejectedTotal.toLocaleString()} (${history.stats.acceptedTotal + history.stats.rejectedTotal > 0 ? ((history.stats.rejectedTotal / (history.stats.acceptedTotal + history.stats.rejectedTotal)) * 100).toFixed(2) : '0'}%)` },
                    ].map(({ label, value }) => (
                      <div key={label}>
                        <div style={labelStyle}>{label}</div>
              ...
              <div style={{ ...cardStyle, textAlign: 'center', padding: 40, color: '#52525b' }}>
                {t('Aucune donnée de télémétrie sur cette plage.', 'No telemetry data for this range.')}
              </div>
            )}
          </>
        )}
        ...
        {tab === 'timeline' && (
          <div style={cardStyle}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div>
                <h3 style={{ fontSize: 15, fontWeight: 600, color: '#fafafa', margin: 0 }}>{t('Historique des événements', 'Event history')}</h3>
                <p style={{ fontSize: 12.5, color: '#71717a', margin: '6px 0 0' }}>{t('Transitions système, alertes, actions manuelles et maintenance auto.', 'System transitions, alerts, manual actions and auto maintenance.')}</p>
              </div>
              <span style={{ fontSize: 12, color: '#52525b' }}>{t(`${events.length} événements`, `${events.length} events`)}</span>
            </div>
            ...
            {eventsLoading && events.length === 0 ? (
              ...
              <div style={{ textAlign: 'center', padding: 40, color: '#52525b' }}>{t('Aucun événement enregistré pour ce mineur.', 'No event recorded for this miner.')}</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {tab === 'settings' && (
          <div style={cardStyle}>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: '#fafafa', marginBottom: 18 }}>{t('Réglages du mineur', 'Miner settings')}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                { label: t('Nom', 'Name'), key: 'name' as const, placeholder: t('Nom du mineur', 'Miner name') },
