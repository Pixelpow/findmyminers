...
const COLS = [
  { key: 'name', label: 'Nom', labelEn: 'Name', sortable: true },
  { key: 'algorithm', label: 'Algo', labelEn: 'Algo', sortable: true },
  { key: 'url', label: 'Endpoint', labelEn: 'Endpoint', sortable: false },
  { key: 'username', label: 'Worker', labelEn: 'Worker', sortable: false },
  { key: 'password', label: 'Auth', labelEn: 'Auth', sortable: false },
  { key: 'ping', label: 'Latence', labelEn: 'Latency', sortable: true },
  { key: 'apply', label: '', labelEn: '', sortable: false },
];
...
const KNOWN_SOLO_POOLS: Array<{
  ...
  { name: 'FindMyBlock · BTC', url: 'stratum+tcp://eu.findmyblock.xyz:3335', coin: 'BTC', region: 'FR', fees: '0 %', note: 'Solo Bitcoin — serveur hébergé en France, mineurs domestiques bienvenus', noteEn: 'Solo Bitcoin — server hosted in France, home miners welcome', featured: true, badge: { label: 'RECOMMANDÉ FR', tone: 'btc' } },
  { name: 'FindMyBlock · BCH', url: 'stratum+tcp://eu.bch.findmyblock.xyz:4335', coin: 'BCH', region: 'FR', fees: '0 %', note: 'Solo Bitcoin Cash — même infra française (secours : eu.molepool.com:5566)', noteEn: 'Solo Bitcoin Cash — same French infra (backup: eu.molepool.com:5566)', featured: true, badge: { label: 'RECOMMANDÉ FR', tone: 'btc' } },
  { name: 'Les Chauffagistes 🇫🇷', url: 'stratum+tcp://chauffagistes-pool.fr:3333', coin: 'BTC', region: 'FR', fees: '0 %', note: 'Pool communautaire français des mineurs-chauffagistes — financé par dons', noteEn: 'French community pool by the “heater miners” — donation-funded', featured: true, badge: { label: 'RECOMMANDÉ FR', tone: 'btc' } },
  { name: 'Public Pool', url: 'stratum+tcp://public-pool.io:21496', coin: 'BTC', region: 'Monde', fees: '0 %', note: 'Open source, le plus populaire chez les Bitaxe / NerdMiner (serveur US)', noteEn: 'Open source, the most popular among Bitaxe / NerdMiner (US server)' },
  { name: 'Braiins Solo', url: 'stratum+tcp://solo.stratum.braiins.com:3333', coin: 'BTC', region: 'EU', fees: '0,5 %', note: 'Infra pro Braiins (Tchéquie) — ports 3333, 443 ou 25', noteEn: 'Pro Braiins infra (Czech Republic) — ports 3333, 443 or 25' },
  { name: 'ZSolo', url: 'stratum+tcp://btc.zsolo.bid:6057', coin: 'BTC', region: 'EU', fees: '0,5 %', note: 'Pool solo européen — port 6060 pour haute difficulté (NiceHash)', noteEn: 'European solo pool — port 6060 for high difficulty (NiceHash)' },
  { name: 'AtlasPool', url: 'stratum+tcp://solo.atlaspool.io:3333', coin: 'BTC', region: 'Monde', fees: '1,5 %', note: 'Anycast mondial (dont Francfort & Londres) — minimum 400 GH/s', noteEn: 'Global anycast (incl. Frankfurt & London) — minimum 400 GH/s' },
  { name: 'CKPool Solo (Europe)', url: 'stratum+tcp://eusolo.ckpool.org:3333', coin: 'BTC', region: 'EU', fees: '2 %', note: 'Le standard historique du solo (299 blocs depuis 2014) — instance EU', noteEn: 'The historic solo standard (299 blocks since 2014) — EU instance' },
  { name: 'CKPool Solo (US)', url: 'stratum+tcp://solo.ckpool.org:3333', coin: 'BTC', region: 'Monde', fees: '2 %', note: 'Instance principale de CKPool aux États-Unis', noteEn: 'CKPool’s main instance in the United States' },
  { name: 'CKPool Solo (Australie)', url: 'stratum+tcp://ausolo.ckpool.org:3333', coin: 'BTC', region: 'Monde', fees: '2 %', note: 'Instance CKPool Australie', noteEn: 'CKPool Australia instance' },
  { name: 'Solopool.org', url: 'stratum+tcp://btc.solopool.org:8005', coin: 'BTC', region: 'Monde', fees: '1–2 %', note: 'Pool solo multi-coins — vérifie le port exact sur leur page d’aide', noteEn: 'Multi-coin solo pool — check the exact port on their help page' },
];
...
function latencyMeta(value: number | null | undefined, t: (fr: string, en: string) => string) {
  if (value === undefined) return { dot: 'bg-slate-600', text: 'text-slate-500', label: t('non mesuré', 'not measured') };
  if (value === null) return { dot: 'bg-rose-500 dot-glow-rose', text: 'text-rose-400', label: t('INJOIGNABLE', 'UNREACHABLE') };
...
export default function PoolsPage() {
  ...
  const runApply = async () => {
    ...
    if (!applyWorker.trim()) {
      toast('warning', t('Renseigne le wallet.worker avant d’appliquer', 'Fill in wallet.worker before applying'));
      return;
    }
    setApplyBusy(true);
    setApplyResults(null);
    try {
      ...
      if (!res.ok) throw new Error(json.error || t('Application échouée', 'Apply failed'));
      const results = (json.results || []) as ApplyResult[];
      setApplyResults(results);
      const okCount = results.filter((r) => r.ok).length;
      toast(okCount === results.length ? 'success' : okCount > 0 ? 'warning' : 'error',
        t(`Pool appliqué sur ${okCount}/${results.length} mineur${results.length > 1 ? 's' : ''}`,
          `Pool applied to ${okCount}/${results.length} miner${results.length > 1 ? 's' : ''}`));
    } catch (error) {
      toast('error', error instanceof Error ? error.message : t('Application échouée', 'Apply failed'));
    } finally {
      setApplyBusy(false);
    }
  };
      ...
      <section className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        <div className="nova-glass p-4 flex flex-col justify-between transition-transform hover:-translate-y-[1px]">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-3 flex items-center gap-2">
            <ShieldCheck className="w-3.5 h-3.5 text-btc-500" /> {t('Pools enregistrés', 'Saved pools')}
          </div>
          <div className="font-mono text-3xl text-slate-100 font-bold tracking-tight">{pools.length}</div>
          <div className="text-[11px] font-mono text-slate-500 mt-2">{t('Endpoints primaires, secours & test', 'Primary, backup & test endpoints')}</div>
        </div>

        <div className="nova-glass p-4 flex flex-col justify-between transition-transform hover:-translate-y-[1px]">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-3 flex items-center gap-2">
            <TimerReset className="w-3.5 h-3.5 text-btc-500" /> {t('Latence moyenne', 'Average latency')}
          </div>
          ...
          <div className="text-[11px] font-mono text-slate-500 mt-2">
            {measuredPools
              ? t(`${measuredPools} endpoint${measuredPools > 1 ? 's' : ''} mesuré${measuredPools > 1 ? 's' : ''}`, `${measuredPools} endpoint${measuredPools > 1 ? 's' : ''} measured`)
              : t('Lance un ping pour mesurer', 'Run a ping to measure')}
          </div>
        </div>
        ...
        <div className="nova-glass p-4 flex flex-col justify-between transition-transform hover:-translate-y-[1px] col-span-2 xl:col-span-1">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-3 flex items-center gap-2">
            <Activity className="w-3.5 h-3.5 text-btc-500" /> {t('Routes basse latence', 'Low-latency routes')}
          </div>
          <div className={`font-mono text-3xl font-bold tracking-tight ${fastPools > 0 ? 'text-emerald-400' : 'text-slate-100'}`}>{fastPools}</div>
          <div className="text-[11px] font-mono text-slate-500 mt-2">{t('Sous 50 ms aller-retour', 'Under 50 ms round-trip')}</div>
        </div>
      </section>
      ...
      <div className="nova-glass flex flex-col overflow-hidden relative min-h-[420px]">
        ...
        <div className="min-h-14 flex items-center justify-between gap-3 px-5 py-2 border-b border-white/5 bg-white/[0.01] flex-wrap">
          <div className="flex items-center gap-4 flex-wrap">
            <h2 className="text-sm font-semibold text-slate-200 flex items-center gap-2">
              <Droplet className="w-4 h-4 text-btc-500" /> {t('Endpoints de pool', 'Pool endpoints')}
            </h2>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('Rechercher nom, endpoint, worker...', 'Search name, endpoint, worker...')}
            ...
            {selected.size > 0 && (
              ...
              >
                {t('Supprimer', 'Delete')} ({selected.size})
              </button>
            )}
            <button
              type="button"
            ...
            >
              ...
              {pinging ? t('Mesure...', 'Measuring...') : t('Ping des pools', 'Ping pools')}
            </button>
            <button
              type="button"
              ...
              <tr>
                <th className="py-3 px-4 w-10 font-semibold">
                  <input
                    ...
                    aria-label={t('Sélectionner les pools visibles', 'Select visible pools')}
                  />
                </th>
                {COLS.map((column) => (
              ...
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={COLS.length + 1} className="py-24 text-center">
                    ...
                    <h3 className="text-slate-300 font-sans font-semibold mb-1 text-sm">
                      {search ? t('Aucun pool ne correspond à cette recherche', 'No pool matches this search') : t('Aucun endpoint de pool', 'No pool endpoint')}
                    </h3>
                    <p className="text-slate-500 font-sans text-xs mb-4">
                      {search ? t('Essaie une requête plus large ou efface le filtre.', 'Try a broader query or clear the filter.') : t('Ajoute tes endpoints de minage principaux ou de secours pour constituer un catalogue réutilisable.', 'Add your primary or backup mining endpoints to build a reusable catalog.')}
                    </p>
                    {!search && (
                      <button
                        type="button"
                      ...
                      >
                        {t('Ajouter le premier pool', 'Add the first pool')}
                      </button>
                    )}
                  </td>
                </tr>
                ...
                <tr key={pool.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-500">{t('Profil d’endpoint réutilisable', 'Reusable endpoint profile')}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-4 text-right">
                    {(() => {
                      const meta = latencyMeta(pool.ping, t);
                      return (
                  ...
                  <td className="py-3 px-4 text-right">
                    <button
                      ...
                      title={t('Pousser ce pool sur des mineurs de la flotte', 'Push this pool to fleet miners')}
                    >
                      <Send className="w-3 h-3" /> {t('Appliquer', 'Apply')}
                    </button>
                  </td>
                </tr>
              ))}
        ...
        {sorted.length > 0 && (
          <div className="flex items-center justify-between gap-3 px-5 py-3 border-t border-white/5 bg-white/[0.01] flex-wrap font-mono text-[11px] text-slate-500">
            <span>
              {(currentPage - 1) * rowsPerPage + (paginated.length ? 1 : 0)}–{(currentPage - 1) * rowsPerPage + paginated.length} {t('sur', 'of')} {sorted.length}
            </span>
            <div className="flex items-center gap-3 flex-wrap">
      ...
      {showAdd && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-label={t('Ajouter un pool', 'Add a pool')}
          >
            {/* Barre d'accent */}
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/[0.01]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-semibold text-slate-100 m-0 leading-tight">{t('Ajouter un pool', 'Add a pool')}</h2>
                  <p className="text-[11px] text-slate-500 font-sans m-0">
                    {t('Choisis un pool vérifié — latence mesurée en direct — ou saisis ton endpoint', 'Pick a verified pool — latency measured live — or enter your own endpoint')}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="px-6 pt-5">
              <div className="flex items-center justify-between mb-3">
                <div className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold">
                  {t('Pools solo vérifiés', 'Verified solo pools')} <span className="text-btc-500">{t('· FR & 0 % d’abord', '· FR & 0% first')}</span>
                </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[9px] font-mono font-bold px-1.5 py-1 rounded border shrink-0 ${
                        ...
                        {preset.region === 'Monde' ? t('Monde', 'World') : preset.region}
                      </span>
                        ...
                        <span className="block text-[13px] font-sans font-semibold text-slate-100 truncate">
                          ...
                          {preset.badge && (
                            <span className={`ml-2 align-middle text-[8px] font-mono font-bold px-1.5 py-0.5 rounded border ${
                              ...
                              {preset.badge.label === 'RECOMMANDÉ FR' ? t('RECOMMANDÉ FR', 'RECOMMENDED FR') : preset.badge.label}
                            </span>
                          )}
                        </span>
                        <span className="block text-[10.5px] text-slate-500 font-sans truncate mt-0.5">{t(preset.note, preset.noteEn)}</span>
                      </span>
                      <span className="shrink-0 flex flex-col items-end gap-1">
                        <span className="text-[10px] font-mono font-semibold text-slate-400">{t('frais', 'fee')} {preset.fees}</span>
                        {(() => {
                          const ping = preset.url in presetPings ? presetPings[preset.url] : undefined;
                          if (ping === undefined) {
                            return <span className="text-[10px] font-mono text-slate-600 animate-pulse">{t('mesure···', 'pinging···')}</span>;
                          }
                          const meta = latencyMeta(ping, t);
                          return (
                            <span className={`inline-flex items-center gap-1.5 text-[11px] font-mono font-bold ${meta.text}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${meta.dot}`} />
                              {ping === null ? t('HS', 'OFF') : `${ping} ms`}
                            </span>
                          );
                        })()}
                      </span>
              ...
              <p className="text-[10px] text-slate-600 font-sans mt-2.5 mb-0">
                {t('Latences mesurées depuis ton serveur à l’instant. Vérifie l’URL et les frais sur le site officiel du pool avant de miner.', 'Latencies measured from your server right now. Check the URL and fees on the pool’s official site before mining.')}
              </p>
            </div>
            <div className="px-6 py-5 grid grid-cols-2 gap-4">
              {[
                { key: 'name', label: t('Nom du profil', 'Profile name'), placeholder: t('Endpoint solo principal', 'Primary solo endpoint') },
                { key: 'algorithm', label: t('Algorithme', 'Algorithm'), placeholder: 'SHA256' },
                { key: 'url', label: t('URL de l’endpoint', 'Endpoint URL'), placeholder: 'stratum+tcp://pool.example.com:3333', full: true },
                { key: 'username', label: t('Wallet / Worker', 'Wallet / Worker'), placeholder: 'wallet.worker1', full: true },
                { key: 'password', label: t('Mot de passe', 'Password'), placeholder: 'x', full: true },
            ...
            <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-white/5 bg-white/[0.01]">
              <span className="text-[10px] text-slate-600 font-mono">
                {form.url.trim() ? form.url : t('Aucun endpoint sélectionné', 'No endpoint selected')}
              </span>
              <div className="flex gap-2.5">
                ...
                >
                  {t('Enregistrer le pool', 'Save pool')}
                </button>
              </div>
            </div>
          </div>
      ...
      {applyPool && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-label={t('Appliquer le pool aux mineurs', 'Apply pool to miners')}
          >
            <div className="h-1 bg-gradient-to-r from-btc-700 via-btc-500 to-btc-700" />
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/[0.01]">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-base font-semibold text-slate-100 m-0 leading-tight truncate flex items-center gap-2">
                    <CoinIcon coin={guessCoin(applyPool)} size={16} /> {t('Appliquer', 'Apply')} « {applyPool.name} »
                  </h2>
            ...
            <div className="px-6 py-5 space-y-4">
              <label className="grid gap-1.5">
                <span className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">{t('Wallet / Worker poussé sur les mineurs', 'Wallet / Worker pushed to miners')}</span>
                <input
                  value={applyWorker}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">
                    {t('Mineurs cibles', 'Target miners')} ({applySelected.size}/{applyTargets.length})
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    {applySelected.size === applyTargets.length ? t('Tout décocher', 'Deselect all') : t('Tout cocher', 'Select all')}
                  </button>
                </div>
                <div className="grid gap-1 max-h-48 overflow-y-auto pr-1">
                  {applyTargets.length === 0 && (
                    <div className="text-xs text-slate-600 font-sans py-2">{t('Chargement des mineurs...', 'Loading miners...')}</div>
                  )}
                        ...
                        {result && (
                          result.ok ? (
                            <span className="inline-flex items-center gap-1 text-[10px] font-mono text-emerald-400 shrink-0">
                              <CheckCircle2 className="w-3 h-3" /> {result.queued ? t('En file (agent)', 'Queued (agent)') : 'OK'}
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-[10px] font-mono text-rose-400 shrink-0" title={result.error}>
                              <AlertCircle className="w-3 h-3" /> {t('Échec', 'Failed')}
                            </span>
                          )
                        )}
                      </label>
              ...
              <p className="text-[10px] text-amber-400/80 font-sans m-0">
                {t('⚠ Chaque mineur redémarre pour appliquer le nouveau pool — quelques secondes d’interruption du hash.', '⚠ Each miner reboots to apply the new pool — a few seconds of hash interruption.')}
              </p>
            </div>
            ...
            <div className="flex justify-end gap-2.5 px-6 py-4 border-t border-white/5 bg-white/[0.01]">
              ...
              >
                {applyResults ? t('Fermer', 'Close') : t('Annuler', 'Cancel')}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {applyBusy ? t('Application...', 'Applying...') : t(`Appliquer sur ${applySelected.size} mineur${applySelected.size > 1 ? 's' : ''}`, `Apply to ${applySelected.size} miner${applySelected.size > 1 ? 's' : ''}`)}
              </button>
            </div>
          </div>
        </div>
