...
export default function OverclockPage() {
  ...
  const runApply = useCallback(async () => {
    ...
    try {
      ...
      toast(fail ? 'warning' : 'success', `${t('Palier', 'Tier')} ${t(TIER_META[selectedTier].label, TIER_META[selectedTier].labelEn)} ${t('appliqué', 'applied')} : ${ok} OK${fail ? `, ${fail} ${t('en échec', 'failed')}` : ''}`);
    } catch (error) {
  ...
  const saveSchedule = async () => {
    ...
    try {
      ...
      toast('success', schedule.enabled ? t('Planification enregistrée et active', 'Schedule saved and active') : t('Planification enregistrée (désactivée)', 'Schedule saved (disabled)'));
    } catch (error) {
      toast('error', error instanceof Error ? error.message : t('Enregistrement échoué', 'Save failed'));
    } finally {
      ...
      <div className="space-y-6">
          ...
          <div className="relative">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-[13px] text-slate-400">{t('Règle la fréquence (hashrate) et la tension (stabilité) de tes mineurs, ou programme des paliers selon l’heure.', 'Tune your miners’ frequency (hashrate) and voltage (stability), or schedule tiers by time of day.')}</p>
              </div>
              {activeTier && (
                <span className={`ml-auto flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold ${TIER_META[activeTier.tier].chip} ${TIER_META[activeTier.tier].text}`}>
                  {TIER_META[activeTier.tier].emoji} {t('Palier actif', 'Active tier')} : {t(TIER_META[activeTier.tier].label, TIER_META[activeTier.tier].labelEn)}
                  <span className="text-slate-500 font-normal">({activeTier.source === 'window' ? activeTier.label : t('planning', 'schedule')})</span>
                </span>
              )}
            </div>
            <div className="flex items-start gap-2.5 rounded-xl border border-rose-500/25 bg-rose-500/[0.07] px-4 py-3 max-w-3xl">
              ...
              <p className="text-[12.5px] text-rose-200/90 leading-relaxed">
                {t(
                  'Réglages avancés, à tes risques. Une tension trop élevée ou un refroidissement insuffisant peut réduire la durée de vie des puces, voire endommager définitivement ton mineur. N’utilise cette fonction que si tu sais ce que tu fais — tu restes responsable des valeurs appliquées. Monte par paliers et surveille la température.',
                  'Advanced settings, at your own risk. Too high a voltage or insufficient cooling can shorten chip lifespan, or permanently damage your miner. Only use this if you know what you’re doing — you remain responsible for the values you apply. Step up gradually and watch the temperature.',
                )}
              </p>
            </div>
          </div>
        ...
        <section className="nova-glass rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-slate-200">{t('Application rapide', 'Quick apply')}</h3>
            <span className="text-xs text-slate-500">{t('choisis un palier, applique-le à la sélection', 'pick a tier, apply it to the selection')}</span>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-sm font-semibold ${active ? meta.text : 'text-slate-200'}`}>{t(meta.label, meta.labelEn)}</span>
                    {meta.danger && <AlertTriangle className="w-3.5 h-3.5 text-rose-400 ml-auto" />}
                  </div>
                  <p className="text-[11px] text-slate-500 leading-snug">{t(meta.short, meta.shortEn)}</p>
                </button>
              );
            })}
          </div>

          <p className="text-[12px] text-slate-400 mb-4 leading-relaxed">{t(TIER_META[selectedTier].desc, TIER_META[selectedTier].descEn)}</p>

          {/* Liste des mineurs */}
          {miners.length === 0 ? (
            <div className="text-sm text-slate-500 py-8 text-center border border-dashed border-white/10 rounded-xl">
              {t('Aucun mineur pilotable détecté. Les Bitaxe (AxeOS) et ASIC CGMiner apparaissent ici une fois en ligne.', 'No controllable miner detected. Bitaxe (AxeOS) and CGMiner ASICs show up here once online.')}
            </div>
          ) : (
            <div className="rounded-xl border border-white/5 overflow-hidden">
              <div className="flex items-center gap-3 px-4 py-2.5 bg-white/[0.02] border-b border-white/5 text-[11px] uppercase tracking-wider text-slate-500 font-semibold">
                ...
                <span className="flex-1">{t('Mineur', 'Miner')}</span>
                <span className="w-24 hidden sm:block">{t('Puce', 'Chip')}</span>
                <span className="w-28 text-right">{t('Actuel', 'Current')}</span>
                <span className="w-32 text-right">{t('Cible', 'Target')} ({t(TIER_META[selectedTier].label, TIER_META[selectedTier].labelEn)})</span>
              </div>
              {preview.length === 0 && (
                <div className="px-4 py-6 text-center text-[13px] text-slate-500">{t('Coche au moins un mineur pour voir l’aperçu.', 'Tick at least one miner to see the preview.')}</div>
              )}
              {miners.map((m) => {
                const chip = detectChipFamily({ chipType: m.chipType, model: m.model });
                const support = ocSupport(m);
                  ...
                  <div key={m.id} className={`flex items-center gap-3 px-4 py-3 border-b border-white/5 last:border-0 transition-colors ${isSel ? 'bg-btc-500/[0.04]' : 'hover:bg-white/[0.02]'}`}>
                    ...
                    <span className="w-32 text-right text-[12px] font-mono">
                      {support === 'freqvolt'
                        ...
                        : <span className="text-slate-300">mode {selectedTier === 'eco' ? t('éco', 'eco') : selectedTier === 'balanced' ? 'normal' : 'perf'}</span>}
                    </span>
                  </div>
                );
              })}
          ...
          {selectedMiners.length > 0 && (
            <div className="flex flex-wrap items-center justify-between gap-3 mt-4">
              ...
              >
                {applying ? t('Application…', 'Applying…') : `${t('Appliquer', 'Apply')} ${TIER_META[selectedTier].label} ${t('à', 'to')} ${selectedMiners.length} ${t('mineur(s)', 'miner(s)')}`}
              </button>
            </div>
          )}
        </section>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <section className="nova-glass rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-slate-200">{t('Catalogue par puce', 'Per-chip catalog')}</h3>
            </div>
            ...
            <p className="text-[11px] text-slate-500 mt-4 leading-relaxed">
              {t('Valeurs par puce (Solo Satoshi / D-Central), tension bornée à la limite 24/7. Le hashrate réel dépend du refroidissement et du silicium.', 'Per-chip values (Solo Satoshi / D-Central), voltage capped to the 24/7 limit. Real hashrate depends on cooling and silicon.')}
            </p>
          </section>
          ...
          <section className="nova-glass rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-slate-200">{t('Réglage manuel (expert)', 'Manual tuning (expert)')}</h3>
            </div>
            {freqVoltMiners.length === 0 ? (
              <div className="text-sm text-slate-500 py-8 text-center border border-dashed border-white/10 rounded-xl">
                {t('Aucun mineur AxeOS (freq/voltage) en ligne. Le réglage manuel n’est disponible que pour les Bitaxe.', 'No AxeOS miner (freq/voltage) online. Manual tuning is only available for Bitaxe devices.')}
              </div>
            ) : (
              <div className="space-y-5">
                <div>
                  <label className="block text-[12px] text-slate-400 mb-1.5">{t('Mineur', 'Miner')}</label>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-[12px] text-slate-400">{t('Fréquence', 'Frequency')}</label>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-[12px] text-slate-400">{t('Tension cœur', 'Core voltage')}</label>
                ...
                {manualVolt > manualMaxV - 50 && (
                  <div className="flex items-start gap-2 text-[11px] text-rose-300 bg-rose-500/10 border border-rose-500/20 rounded-lg p-2.5">
                    ...
                    {t('Tu approches de la tension maximale sûre de cette puce. Assure-toi d’un refroidissement renforcé.', 'You’re nearing this chip’s safe voltage limit. Make sure cooling is reinforced.')}
                  </div>
                )}
                ...
                <div className="flex items-center gap-2">
                  <button onClick={runManual} disabled={manualBusy} className="focus-ring flex-1 px-4 py-2 rounded-lg text-sm font-semibold bg-btc-500/15 text-btc-400 border border-btc-500/30 hover:bg-btc-500/25 transition-colors disabled:opacity-50">
                    {manualBusy ? t('Application…', 'Applying…') : t('Appliquer à ce mineur', 'Apply to this miner')}
                  </button>
                  <button onClick={resetStock} className="focus-ring px-3 py-2 rounded-lg text-sm text-slate-400 border border-white/10 hover:text-slate-200 hover:bg-white/[0.04] transition-colors" title={t('Revenir aux valeurs d’usine', 'Reset to stock values')}>
        ...
        <section className="nova-glass rounded-2xl p-6">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <h3 className="text-sm font-semibold text-slate-200">{t('Planification horaire', 'Time scheduling')}</h3>
            <label className="ml-auto flex items-center gap-2 text-[13px] text-slate-300 cursor-pointer">
              ...
              {t('Activer le planning', 'Enable schedule')}
            </label>
          </div>
          <p className="text-[12px] text-slate-400 mb-4 leading-relaxed max-w-3xl">
            {t('Programme des créneaux (ex. Éco la nuit, Turbo en journée). Le palier est appliqué automatiquement côté serveur — même navigateur fermé. Hors créneau, le palier par défaut s’applique. Le mode vacances suspend le planning.', 'Schedule time windows (e.g. Eco at night, Turbo during the day). The tier is applied automatically server-side — even with the browser closed. Outside any window, the default tier applies. Vacation mode pauses the schedule.')}
          </p>

          {/* Timeline 24h */}
          <div className="mb-5">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[11px] text-slate-500">{t('Aperçu', 'Preview')}</span>
              <select value={previewDay} onChange={(e) => setPreviewDay(Number(e.target.value))} className="bg-obsidian-950 border border-white/10 rounded-md py-0.5 px-1.5 text-[11px] text-slate-300">
                {DAY_LABELS.map((d, i) => <option key={i} value={i} style={{ background: '#0a0a0c' }}>{t(d, DAY_LABELS_EN[i])}</option>)}
              </select>
            </div>
          ...
          {/* Palier par défaut */}
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="text-[12px] text-slate-400">{t('Hors créneau', 'Outside windows')} :</span>
              ...
              <div key={w.id} className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                <div className="flex flex-wrap items-end gap-3">
                  <div className="flex-1 min-w-[140px]">
                    <label className="block text-[11px] text-slate-500 mb-1">{t('Nom', 'Name')}</label>
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-500 mb-1">{t('De', 'From')}</label>
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-500 mb-1">{t('À', 'To')}</label>
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-500 mb-1">{t('Palier', 'Tier')}</label>
                  ...
                  <div className="w-20">
                    <label className="block text-[11px] text-slate-500 mb-1">{t('Ventilo %', 'Fan %')}</label>
                  ...
                  <button onClick={() => removeWindow(w.id)} className="focus-ring p-2 rounded-lg text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 transition-colors" title={t('Supprimer ce créneau', 'Remove this window')}>
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                <div className="flex items-center gap-1.5 mt-3">
                  <span className="text-[11px] text-slate-500 mr-1">{t('Jours', 'Days')} :</span>
                  {DAY_LABELS.map((d, i) => {
                    const on = w.days.length === 0 || w.days.includes(i);
                    const allDays = w.days.length === 0;
                    return (
                      <button key={i} onClick={() => toggleWindowDay(w.id, i)} className={`focus-ring px-2 py-0.5 rounded text-[11px] border transition-colors ${on ? 'bg-btc-500/10 border-btc-500/25 text-btc-400' : 'border-white/5 text-slate-500 hover:bg-white/[0.04]'}`} title={allDays ? t('Tous les jours (clique pour restreindre)', 'Every day (click to restrict)') : undefined}>
                        {t(d, DAY_LABELS_EN[i])}
                      </button>
                    );
                  })}
                  {w.days.length === 0 && <span className="text-[10px] text-slate-600 ml-1">{t('tous les jours', 'every day')}</span>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-4">
            <button onClick={addWindow} className="focus-ring inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-[13px] text-slate-300 border border-white/10 hover:bg-white/[0.04] transition-colors">
              <Plus className="w-4 h-4" /> {t('Ajouter un créneau', 'Add a window')}
            </button>
            <button onClick={saveSchedule} disabled={savingSchedule} className="focus-ring inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-semibold bg-btc-500/15 text-btc-400 border border-btc-500/30 hover:bg-btc-500/25 transition-colors disabled:opacity-50 ml-auto">
              <Save className="w-4 h-4" /> {savingSchedule ? t('Enregistrement…', 'Saving…') : t('Enregistrer la planification', 'Save schedule')}
            </button>
          </div>
        </section>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" data-modal-overlay onClick={() => setConfirmApply(false)}>
          ...
          <div className="relative w-full max-w-lg nova-glass rounded-2xl p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-base font-semibold text-slate-100">{t('Confirmer l’application', 'Confirm apply')}</h3>
            ...
            <p className="text-[13px] text-slate-400 mb-4">
              {t('Palier', 'Tier')} <span className={`font-semibold ${TIER_META[selectedTier].text}`}>{TIER_META[selectedTier].emoji} {TIER_META[selectedTier].label}</span> {t('sur', 'on')} <strong className="text-slate-200">{selectedMiners.length}</strong> {t('mineur(s)', 'miner(s)')}.
              {selectedTier === 'extreme' && <span className="block mt-1 text-rose-300">{t('⚠ Palier extrême : refroidissement renforcé requis, risque d’usure accélérée.', '⚠ Extreme tier: reinforced cooling required, risk of accelerated wear.')}</span>}
            </p>
              ...
              {preview.map((p) => (
                <div key={p.miner.id} className="flex items-center justify-between gap-2 px-3 py-2 text-[12px]">
                  ...
                  <span className="font-mono text-slate-400 shrink-0">
                    {p.support === 'freqvolt' ? `${p.freqMHz} MHz / ${p.coreVoltageMV} mV` : `mode ${selectedTier === 'eco' ? t('éco', 'eco') : selectedTier === 'balanced' ? 'normal' : 'perf'}`}
                  </span>
                </div>
              ))}
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirmApply(false)} className="focus-ring px-4 py-2 rounded-lg text-sm text-slate-300 border border-white/10 hover:bg-white/[0.04]">{t('Annuler', 'Cancel')}</button>
              <button onClick={() => void runApply()} className={`focus-ring px-4 py-2 rounded-lg text-sm font-semibold ${selectedTier === 'extreme' ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 hover:bg-rose-500/30' : 'bg-btc-500/20 text-btc-300 border border-btc-500/40 hover:bg-btc-500/30'}`}>{t('Confirmer', 'Confirm')}</button>
            </div>
          </div>
        </div>
      )}
