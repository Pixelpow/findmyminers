...
export async function evaluateAndNotifyAlertsV2(
  ...
  if ((snapshot.tempAvg >= thermalThreshold || snapshot.tempMax >= thermalThreshold + 5) && shouldNotify('thermal', `thermal-${miner.id}`)) {
    const msg = [
      `🚨 ${miner.name} Thermal Alert: ${snapshot.tempAvg.toFixed(1)}°C (max ${snapshot.tempMax.toFixed(1)}°C)`,
  ...
  if (avgHashrateTHs > 0 && snapshot.hashrateTHs < avgHashrateTHs * hashrateDropRatio && shouldNotify('hashrate-drop', `hashrate-drop-${miner.id}`)) {
    const msg = [
      `⚠️ ${miner.name} Hashrate Drop: ${snapshot.hashrateTHs.toFixed(2)} TH/s (recent avg ${avgHashrateTHs.toFixed(2)} TH/s)`,
  ...
  if (!snapshot.poolAlive && shouldNotify('pool-down', `pool-down-${miner.id}`)) {
    ...
    const msg = `⛔ ${miner.name} Pool Down: no active pool detected. Likely cause: upstream pool outage, DNS issue, or local network instability.${failoverMsg}`;
    await sendAlert(msg, settings);
  ...
  if (longAvgHashrateTHs > 0 && snapshot.hashrateTHs < longAvgHashrateTHs * anomalyDropRatio && shouldNotify('anomaly', `anomaly-${miner.id}`)) {
    const msg = [
      `🧠 ${miner.name} Anomaly detected: hashrate ${snapshot.hashrateTHs.toFixed(2)} TH/s vs baseline ${longAvgHashrateTHs.toFixed(2)} TH/s.`,
      inferHashrateCause(snapshot, avgHashrateTHs, longAvgHashrateTHs),
    ].join(' ');
    await sendAlert(msg, settings);
...
export async function maybeSendDailyReport(
  ...
  const message = [
    `📊 Daily report ${miner.name}`,
    `• Avg hashrate: ${stats.avgHashrate.toFixed(2)} TH/s`,
    `• Avg temp: ${stats.avgTemp.toFixed(1)} °C`,
    `• Pool uptime: ${(stats.uptimeRatio * 100).toFixed(1)} %`,
    `• Est. net: ${stats.netEurDaily >= 0 ? '+' : ''}${stats.netEurDaily.toFixed(2)} €/d`,
  ].join('\n');
