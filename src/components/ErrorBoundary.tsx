...
export default class ErrorBoundary extends React.Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      // Class component: no hooks, so read the saved language directly.
      let fr = false;
      try { fr = localStorage.getItem('fmm_lang') === 'fr'; } catch { /* ignore */ }
      return (
        <div style={{
          ...
          <p style={{ fontSize: 14, color: 'var(--muted)', marginBottom: 20, maxWidth: 420, textAlign: 'center' }}>
            {this.state.error?.message || (fr ? 'Erreur inattendue.' : 'Unexpected error.')}
          </p>
          <button
