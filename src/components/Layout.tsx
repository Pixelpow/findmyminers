...
const NAV_ITEMS: NavItem[] = [
  // Vue d'ensemble
  { label: 'Tableau de bord', labelEn: 'Dashboard', href: '/dashboard', icon: LayoutGrid },
  // Pilotage
  { label: 'Overclock', labelEn: 'Overclock', href: '/overclock', icon: Gauge },
  { label: 'Pools', labelEn: 'Pools', href: '/pools', icon: Droplet },
  // Analyse
  { label: 'Conseiller', labelEn: 'Advisor', href: '/advisor', icon: Bot, badgeKey: 'criticalAdvice', badgeTone: 'blue' },
  { label: 'Records', labelEn: 'Records', href: '/records', icon: BarChart3 },
];

/** Titres du header par préfixe de route : [FR, EN]. */
const PAGE_TITLES: Record<string, [string, string]> = {
  dashboard: ['Tableau de bord', 'Dashboard'],
  miners: ['Mineurs', 'Miners'],
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const [orgName, setOrgName] = useState('FindMyMiners');
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setOrgName(activeOrg?.name || 'FindMyMiners');
  ...
  const titlePair = PAGE_TITLES[firstSegment];
  const pageTitle = titlePair ? t(titlePair[0], titlePair[1]) : firstSegment.charAt(0).toUpperCase() + firstSegment.slice(1);
      ...
      <aside className={`w-64 fixed flex flex-col left-0 top-0 bottom-0 bg-obsidian-900 border-r border-white/5 z-40 transition-transform ${mobileNavOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-medium">{t(item.label, item.labelEn)}</span>
                {badgeCount > 0 && (
        ...
        <div className="px-4 py-6 border-t border-white/5 space-y-1">
          ...
          <Link href="/settings" className="focus-ring flex items-center gap-3 px-3 py-2 rounded-lg text-slate-500 hover:text-slate-300 transition-colors">
            ...
            <span className="text-sm">{t('Paramètres', 'Settings')}</span>
          </Link>
          {authUser && (
            <button
              type="button"
            ...
            >
              ...
              <span className="text-sm">{t('Déconnexion', 'Log out')}</span>
            </button>
          )}
          {/* Méta : liens discrets, séparés par un filet */}
          <div className="mt-2 pt-3 px-1 border-t border-white/5 space-y-2 text-[10px] text-slate-600">
            <div className="flex items-center gap-3">
              <a
                href="https://github.com/Pixelpow/findmyminers"
                target="_blank"
                rel="noopener noreferrer"
                className="focus-ring hover:text-slate-200 transition-colors"
                title="GitHub"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="focus-ring inline-flex items-center gap-1 hover:text-btc-500 transition-colors"
                title="Pixel sur X"
              >
                <Twitter className="w-3 h-3" />
                {t('par Pixel', 'by Pixel')}
              </a>
            </div>
            <a
              href="https://mineshop.eu/?wpam_id=35"
              target="_blank"
              rel="noopener noreferrer"
              className="focus-ring inline-flex items-center gap-1 hover:text-btc-500 transition-colors"
              title="MineShop"
            >
              ...
              {t('Besoin d’un mineur ?', 'Need a miner?')}
            </a>
          </div>
        </div>
      </aside>
            ...
            <div className="flex items-center gap-5">
              {backendDown ? (
                <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-md bg-rose-500/10 border border-rose-500/20">
                  ...
                  <span className="text-[11px] font-mono font-medium text-rose-400 tracking-wide">{t('SERVEUR HORS LIGNE', 'SERVER OFFLINE')}</span>
                </div>
              ) : (
                <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-md bg-emerald-500/10 border border-emerald-500/20">
                  ...
                  <span className="text-[11px] font-mono font-medium text-emerald-400 tracking-wide">{t('MAINNET SYNCHRONISÉ', 'MAINNET SYNCED')}</span>
                </div>
              )}
          ...
          <div className="py-2.5 px-6 lg:px-8 border-t border-white/5 bg-white/[0.01] overflow-x-auto">
            <div className="flex items-center space-x-8 text-[13px] whitespace-nowrap min-w-max">
              <div className="flex items-baseline gap-2">
                <span className="text-slate-500 uppercase text-[10px] tracking-wider font-semibold">{t('Prix BTC', 'BTC price')}</span>
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-slate-500 uppercase text-[10px] tracking-wider font-semibold">{t('Difficulté', 'Difficulty')}</span>
              ...
              {showProfit && (
                <>
                  <div className="w-px h-4 bg-white/10" />
                  <div className="flex items-baseline gap-2">
                    <span className="text-slate-500 uppercase text-[10px] tracking-wider font-semibold">{t('Est. flotte/jour', 'Est. fleet/day')}</span>
                    <span className="font-mono text-emerald-400">{fmtEur0(totals?.dailyNetEur)}</span>
                  </div>
                  <div className="w-px h-4 bg-white/10" />
                  <div className="flex items-baseline gap-2">
                    <span className="text-slate-500 uppercase text-[10px] tracking-wider font-semibold">{t('Est. flotte/mois', 'Est. fleet/month')}</span>
                    <span className="font-mono text-emerald-400">{fmtEur0(totals?.monthlyNetEur)}</span>
                  </div>
                  <div className="w-px h-4 bg-white/10" />
                  <div className="flex items-baseline gap-2">
                    <span className="text-slate-500 uppercase text-[10px] tracking-wider font-semibold">{t('Élec moy.', 'Avg. power')}</span>
                    <span className="font-mono text-slate-200">
                  ...
                  <div className="w-px h-4 bg-white/10" />
                  <div className="flex items-baseline gap-2">
                    <span className="text-slate-500 uppercase text-[10px] tracking-wider font-semibold">{t('Frais pool moy.', 'Avg. pool fee')}</span>
                    <span className="font-mono text-slate-200">
